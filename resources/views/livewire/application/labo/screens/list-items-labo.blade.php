                            ...
                            <tr>
                                ...
                                <td class="text-xl-center">
                                    @if ($isEditing && $idTarif == $tarif->id)
                                        <x-form.input type='number' style="width: 70px;text-align: center"
                                            wire:model='qty' wire:keydown.enter='update' :error="'qty'" />
                                    @else
                                        {{ $tarif->qty }}
                                    @endif
                                </td>
                                <td class="text-center">
                                    @if ($isEditing && $idTarif == $tarif->id)
                                        <textarea class="form-control" rows="2" wire:model='result'>
                                        </textarea>
                                    @else
                                ...
                                <td class="text-center">
                                    @if ($isEditing && $idTarif == $tarif->id)
                                        <x-form.input type='text' wire:model='normal_value'
                                            wire:keydown.enter='update' :error="'normal_value'" />
                                    @else
                                        {{ $tarif->normal_value == null ? 'Aucun' : $tarif->normal_value }}
                                    @endif
                                </td>
                                <td class="text-center">
                                    @if ($isEditing && $idTarif == $tarif->id)
                                        <x-form.input type='text' wire:model='unit' wire:keydown.enter='update'
                                            :error="'unit'" />
                                    @else
                                        {{ $tarif->unit == null ? 'Aucun' : $tarif->unit }}
                                    @endif
                                </td>
                                <td class="text-center">
